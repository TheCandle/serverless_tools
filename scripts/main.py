...
from utils import setup_environment, validate_experiment_config, create_dataset_loader, setup_file_logging

RUN_PRESTO = True

def export_and_plot_operator_dop_time(
    dataset: str,
    train_mode: str,
    train_ratio: float = 1.0,
    use_estimates_mode: bool = False,
):
    """对测试集每一行(PlanNode)用已训练DOP-aware模型预测参数(a,b,c,d,e)，并画 pred_time-DOP 曲线。"""
    from config.structure_config import dop_operator_features
    from utils import get_model_paths

    loader = create_dataset_loader(dataset)
    test_df = loader.load_test_data(use_estimates_mode, train_ratio=train_ratio)
    if test_df is None or test_df.empty:
        print("❌ 测试集为空，无法绘图")
        return False

    if 'operator_type' not in test_df.columns:
        print("❌ 测试集缺少 operator_type 列")
        return False

    model_dir = get_model_paths(dataset, train_mode, 'dop_aware')['model_dir']

    output_dir = os.path.join(os.path.dirname(__file__), '..', 'output', dataset, 'analysis', train_mode)
    curves_dir = os.path.join(output_dir, 'operator_curve_per_row')
    os.makedirs(curves_dir, exist_ok=True)

    dop_grid = np.array([1, 2, 4, 8, 16, 32, 64, 96], dtype=np.float32)
    all_rows = []

    operators = sorted(test_df['operator_type'].dropna().unique().tolist())
    for op in operators:
        if op not in dop_operator_features or 'exec' not in dop_operator_features[op]:
            continue

        feature_cols = dop_operator_features[op]['exec']
        op_df = test_df[test_df['operator_type'] == op].copy()
        if op_df.empty:
            continue

        missing = [c for c in feature_cols if c not in op_df.columns]
        if missing:
            print(f"⚠️ 跳过算子 {op}: 缺少特征列 {missing}")
            continue

        onnx_path = os.path.join(model_dir, op, f"exec_{op.replace(' ', '_')}.onnx")
        if not os.path.exists(onnx_path):
            print(f"⚠️ 跳过算子 {op}: 模型不存在 {onnx_path}")
            continue

        X = op_df[feature_cols].replace([np.inf, -np.inf], np.nan).dropna()
        if X.empty:
            continue

        session = ort.InferenceSession(onnx_path)
        input_name = session.get_inputs()[0].name
        params = session.run(None, {input_name: X.values.astype(np.float32)})[0]  # [N,5]

        fig, ax = plt.subplots(figsize=(8, 5))
        for i in range(params.shape[0]):
            a, b, c, d, e = params[i]
            pred = b / (dop_grid ** a) + c * (dop_grid ** d) + e
            pred = np.maximum(pred, 0.0)
            row_id = int(X.index[i])

            for j, dop in enumerate(dop_grid):
                all_rows.append({
                    'operator_type': op,
                    'row_id': row_id,
                    'dop': float(dop),
                    ...
                    'd': float(d),
                    'e': float(e),
                    'pred_execution_time': float(pred[j]),
                })

            ax.plot(dop_grid, pred, alpha=0.15, linewidth=1)

        ax.set_title(f"{op} - per-row predicted curves")
        ax.set_xlabel('DOP')
        ax.set_ylabel('Predicted Execution Time')
        ax.grid(alpha=0.3)
        ax.set_xscale('log', base=2)
        fig.tight_layout()
        fig_path = os.path.join(curves_dir, f"{op.replace('/', '_').replace(' ', '_')}_pred_curve.png")
        fig.savefig(fig_path, dpi=180)
        plt.close(fig)

    if not all_rows:
        print("❌ 没有可导出的曲线数据（可能模型缺失或特征列不匹配）")
        return False

    out_csv = os.path.join(output_dir, 'operator_dop_pred_curve_per_row.csv')
    pd.DataFrame(all_rows).to_csv(out_csv, index=False)
    print("✅ 已生成每个算子、每行数据的 DOP-预测时间曲线")
    print(f"   曲线明细: {out_csv}")
    print(f"   图目录: {curves_dir}")
    return True
...
def main():
    ...
    RUN_EXPORT_OPERATOR_DOP_TIME = False  # Whether to export and plot operator execution time under different DOP
    # =======================================================
    ...
    # Setup file logging
    setup_file_logging(log_dir='logs', log_prefix=f"main_{TRAIN_METHOD}_{DATASET}_{TRAIN_MODE}")
    ...
    # Export operator execution time by DOP and plot
    if RUN_EXPORT_OPERATOR_DOP_TIME:
        print(f"\n🧩 Exporting operator execution time under different DOP")
        success = export_and_plot_operator_dop_time(
            DATASET,
            TRAIN_MODE,
            train_ratio=TRAIN_RATIO,
            use_estimates_mode=USE_ESTIMATES_MODE,
        )
        if success:
            print("✅ Operator-DOP export & plotting completed")
        else:
            print("❌ Operator-DOP export & plotting failed")
            return
