...
def prepare_data(train_data, test_data, operator, feature_columns, target_columns, epsilon=1e-5, use_estimates=False, return_group_cols=False): # <-- 1. 增加 use_estimates 开关
    ...
    # 可选返回分组列，用于同一曲线上的 dop 梯度监督
    group_cols = ['query_num', 'plan_id', 'round_num']
    if return_group_cols:
        for gc in group_cols:
            if gc not in train_data.columns:
                train_data[gc] = -1
            if gc not in test_data.columns:
                test_data[gc] = -1
        group_train = train_data[group_cols].copy()
        group_test = test_data[group_cols].copy()
