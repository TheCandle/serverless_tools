...
def build_segments_from_bottom_up(all_nodes: List[PlanNode], thread_blocks: Dict[int, ThreadBlock]) -> Segment:
    ...
    def _node_is_stage_boundary(node: PlanNode) -> bool:
        """Check if node is a stage-boundary operator."""
        return is_stage_boundary_operator(getattr(node, 'operator_type', ''))
    
    def _stage_critical_path_latency(stage_nodes: List[PlanNode]) -> float:
        """Estimate stage latency by longest path on child->parent edges inside stage nodes."""
        if not stage_nodes:
            return 0.0
        ...
        def node_cost(n: PlanNode) -> float:
            return float(getattr(n, 'execution_time', 0.0) or 0.0)

        def dfs(cur: PlanNode) -> float:
            key = id(cur)
            if key in memo:
                return memo[key]

            parent = getattr(cur, 'parent_node', None)
            if parent is not None and parent in stage_set:
                best = node_cost(cur) + dfs(parent)
            else:
                best = node_cost(cur)
        ...
        return max(dfs(n) for n in stage_nodes)
    ...
    def traverse_upward_to_breaker(start_node: PlanNode) -> Tuple[List[PlanNode], Optional[PlanNode], bool]:
        ...
        while current.parent_node:
            ...
            if is_pipeline_breaker_operator(getattr(parent, 'operator_type', '')) or bool(getattr(parent, 'materialized', False)):
                # Found a breaker, include it in the segment
                nodes_in_segment.append(parent)
                print("==============================breaker===============================")
                print(parent.operator_type)
                print("==============================breaker===============================")
                # Check if we crossed a stage boundary
                parent_stage_id = node_to_stage.get(parent, 0)
                if parent_stage_id != start_stage_id or _node_is_stage_boundary(current):
                    is_cross_stage = True
    ...
    while queue:
        ...
        # For each stage, split by pipeline breaker and use critical-path approximation:
        # sum over per-pipeline max(branch_cost).
        pipeline_latencies = []
        dop_info = {}
        for stage_id in stage_ids_in_segment:
            ...
            if tb and tb.nodes:
                ...
                if stage_nodes:
                    breaker_nodes = [
                        n for n in stage_nodes
                        if is_pipeline_breaker_operator(getattr(n, 'operator_type', '')) or bool(getattr(n, 'materialized', False))
                    ]
                    # Stage-internal pipeline parallelism approximation: longest path in this stage.
                    stage_pipeline_latency = _stage_critical_path_latency(stage_nodes)
                    pipeline_latencies.append(stage_pipeline_latency)
                    dop_info[stage_id] = {
                        'dop': stage_nodes[0].dop if stage_nodes else 1,
                        'nodes': stage_nodes,
                        'pipeline_breakers': [getattr(n, 'plan_id', None) for n in breaker_nodes],
                        'stage_latency_model': 'critical_path',
                    }
