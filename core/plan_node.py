...
class PlanNode:
    def __init__(self, plan_data, onnx_manager, use_estimates=False):
        ...
        self.stage_id = int(_safe_num(plan_data.get('stage_id', plan_data.get('stageid', 0)), 0))
        self.pipeline_id = int(_safe_num(plan_data.get('pipeline_id', plan_data.get('pipelineid', 0)), 0))
    ...
    def compute_parallel_dop_predictions(self, visited=None, in_stack=None):
        ...
        normalized_dop_sets = sorted({int(d) for d in dop_sets} | {1})

        if self.is_parallel and self.pred_params is not None:
            for dop in normalized_dop_sets:
                pred_exec = (self.pred_params[1] / (dop ** self.pred_params[0])
                            + self.pred_params[2] * (dop ** self.pred_params[3])
                            + self.pred_params[4])
