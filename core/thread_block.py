...
class ThreadBlock:
    ...
    def aggregate_metrics(self):
        ...
        if getattr(self, 'debug_inf_trace', False):
            pred_items = list(self.pred_dop_exec_time.items()) if isinstance(self.pred_dop_exec_time, dict) else []
            invalid_items = []
            for k, v in pred_items:
                try:
                    ...
                    invalid_items.append((k, v))
            print(f"[INF_TRACE][aggregate_metrics] tb={self.thread_id} top_node_plan={getattr(top_node, 'plan_id', None)} top_node_op={getattr(top_node, 'operator_type', None)} candidate_dops={sorted(self.candidate_dops) if self.candidate_dops else []} pred_keys={sorted(self.pred_dop_exec_time.keys()) if isinstance(self.pred_dop_exec_time, dict) else []} invalid_pred_items={invalid_items[:8]}")
    ...
    def choose_optimal_dop(self,
        ...
        for d in sorted_dops[1:]:
            ...
            if adj_gain >= dynamic_min_improvement:
                ...
                continue
        raw_d_right = d_right
        if d_right < min_candidate_dop:
            d_right = min_candidate_dop
        ...
        if matching_intervals:
            ...
            d_left = sorted_dops[0] if sorted_dops[0] >= min_candidate_dop else min_candidate_dop

        # Ensure left boundary is reasonable
        raw_d_left = d_left
        if d_left < min_candidate_dop:
            d_left = min_candidate_dop
        ...
        if debug_dop:
            try:
                child_times_preview = {k: round(v, 4) for k, v in list(child_upload_times.items())[:5]}
            except Exception:
                child_times_preview = child_upload_times
            print(f"[DEBUG_TB_RANGE] tb={self.thread_id} sorted_dops={sorted_dops} raw_d_left={raw_d_left} raw_d_right={raw_d_right} clamped_d_left={d_left} clamped_d_right={d_right} min_candidate_dop={min_candidate_dop} matching_intervals={matching_intervals} child_upload_times(sample)={child_times_preview}")
            print(f"[DEBUG_TB_RANGE] tb={self.thread_id} final_candidates={candidates}")
    ...
    def compute_consumption_rate(self, dop: int) -> float:
        ...
        if getattr(self, 'debug_inf_trace', False):
            if exec_time is None or (isinstance(exec_time, float) and (math.isnan(exec_time) or math.isinf(exec_time))):
                print(f"[INF_TRACE][consumption] tb={self.thread_id} dop={dop} exec_time={exec_time} blocking={blocking} pred_keys={sorted(self.pred_dop_exec_time.keys()) if isinstance(self.pred_dop_exec_time, dict) else []}")
    ...
    def compute_production_rate(self, dop: int) -> float:
        ...
        if getattr(self, 'debug_inf_trace', False):
            if exec_time is None or (isinstance(exec_time, float) and (math.isnan(exec_time) or math.isinf(exec_time))):
                print(f"[INF_TRACE][production] tb={self.thread_id} dop={dop} exec_time={exec_time} blocking={blocking} pred_keys={sorted(self.pred_dop_exec_time.keys()) if isinstance(self.pred_dop_exec_time, dict) else []}")
    ...
    def choose_optimal_dop_top_down(self,
        ...
        """
        self.debug_inf_trace = debug_inf_trace
        ...
        if has_root_node and force_root_dop_one:
            ...
            for child_id in self.child_thread_ids:
                ...
                if debug_inf_trace and (production_rate is None or (isinstance(production_rate, float) and (math.isnan(production_rate) or math.isinf(production_rate)))):
                    print(f"[INF_TRACE][propagate_root_to_child] parent_tb={self.thread_id} child_tb={child_id} parent_dop=1 production_rate={production_rate}")
                child_tb.choose_optimal_dop_top_down(
                    ...
                    min_candidate_dop=min_candidate_dop,
                    force_root_dop_one=force_root_dop_one,
                    debug_dop=debug_dop,
        ...
        for child_id in self.child_thread_ids:
            ...
            # Propagate production rate downward for downstream to match
            if debug_inf_trace and (production_rate is None or (isinstance(production_rate, float) and (math.isnan(production_rate) or math.isinf(production_rate)))):
                print(f"[INF_TRACE][propagate_non_root_to_child] parent_tb={self.thread_id} child_tb={child_id} parent_dop={self.optimal_dop} production_rate={production_rate}")
            child_tb.choose_optimal_dop_top_down(
                all_thread_blocks,
