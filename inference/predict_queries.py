...
from core.partition_rules import is_stage_boundary_operator
...
def calculate_thread_execution_time(node, thread_id):
    ...
    is_streaming = is_stage_boundary_operator(getattr(node, 'operator_type', ''))
...
def collect_nodes_and_dop_in_path(start_node, end_node):
    ...
    while current and current != end_node:
        ...
        # Check if this is a stage boundary or DOP change
        is_stage_boundary = is_stage_boundary_operator(getattr(current, 'operator_type', '')) if current else False
        dop_changed = (current_dop is not None and node_dop != current_dop)
        
        # Start new stage if stage boundary or DOP changed
        if is_stage_boundary or dop_changed or stage_id == 0:
            if is_stage_boundary or dop_changed:
                stage_id += 1
...
def assign_thread_ids(node, thread_id=0, max_thread_id=0):
    ...
    for child in node.child_plans:
        ...
        if is_stage_boundary_operator(getattr(node, 'operator_type', '')):
...
def build_thread_blocks_from_nodes(all_nodes):
    """
    Build ThreadBlocks from plan nodes using stage_id + pipeline_id.
    Each unique (stage_id, pipeline_id) pair becomes one ThreadBlock.
    """
    if not all_nodes:
        return {}

    def _pipeline_key(node):
        stage_id = getattr(node, 'stage_id', None)
        pipeline_id = getattr(node, 'pipeline_id', None)
        if stage_id is None or pipeline_id is None:
            return ('fallback', getattr(node, 'plan_id', id(node)))
        try:
            return (int(stage_id), int(pipeline_id))
        except (TypeError, ValueError):
            return ('fallback', getattr(node, 'plan_id', id(node)))

    # Assign thread IDs by pipeline key.
    ...
    for node in all_nodes:
        if node.parent_node:
            ...
            if parent_tid != child_tid and parent_tid in thread_blocks:
...
def run_inference(plan_csv_path, query_csv_path, output_csv_path, no_dop_model_dir, dop_model_dir, use_estimates=False):
    ...
    query_trees = {}
    # 采集算子级推理结果（用于按 operator_type + dop 聚合画图）
    operator_level_rows = []
    ...
    for (query_id, query_dop), group in query_groups:
        ...
        # 采集算子级预测结果（用于按 operator_type + dop 聚合画图）
        for n in nodes_dict.values():
            pred_t = getattr(n, 'pred_execution_time', None)
            if pred_t is None:
                pred_t = getattr(n, 'pred_exec_time', None)
    ...
    for (query_id, query_dop), plan_tree in query_trees.items(): # 原始变量名 plan_tree
        ...
        if not RUN_PRESTO:
            ...
            # RUN_PRESTO=True 场景下没有 actual/memory，补齐占位避免 DataFrame 列长度不一致
            query_ids.append(query_id)
            query_dops.append(query_dop)
            predicted_times_in_s.append(predicted_time / 1000.0)
            actual_times_in_s.append(np.nan)
            actual_memories_in_mb.append(np.nan)
            predicted_memories_in_mb.append(np.nan)
            time_q_error.append(np.nan)
            memory_q_error.append(np.nan)
            time_calculation_durations.append(time_calculation_duration)
            memory_calculation_durations.append(np.nan)
    ...
    if operator_level_rows:
        ...
        # 清洗无效行
        df_operator = df_operator.dropna(subset=['operator_type', 'dop', 'pred_execution_time'])
        df_operator.to_csv(output_csv_path, index=False, sep=';')
    else:
        # 回退到查询级结果
