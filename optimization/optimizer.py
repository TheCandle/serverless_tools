...
def compute_optimal_dops_for_query(thread_blocks_for_query, **kwargs):
    ...
    for root_tid in root_thread_ids:
        ...
        root_tb.choose_optimal_dop_top_down(
            ...
            min_candidate_dop=kwargs.get('min_candidate_dop', 8),
            force_root_dop_one=kwargs.get('force_root_dop_one', True),
            debug_dop=kwargs.get('debug_dop', False),
            debug_inf_trace=kwargs.get('debug_inf_trace', False),
        )
...
def run_dop_optimization(df_plans_all_dops, onnx_manager: ONNXModelManager,
    ...
    # Debug switches
    debug_dop = kwargs.get('debug_dop', False)
    debug_query_ids = kwargs.get('debug_query_ids', None)  # e.g. [1, 2]
    debug_max_tb_per_query = kwargs.get('debug_max_tb_per_query', 999999)
    ...
    for query_id, base_nodes in query_trees.items():
        ...
        if debug_dop and _query_id_matches_debug_filter(query_id, debug_query_ids):
            _print_query_dop_debug_snapshot(
                query_id,
                thread_blocks_for_query,
                stage_label='after_compute_optimal_dops_for_query',
                max_tb=debug_max_tb_per_query
            )
        ...
        if debug_dop and _query_id_matches_debug_filter(query_id, debug_query_ids):
            _print_query_dop_debug_snapshot(
                query_id,
                thread_blocks_for_query,
                stage_label='before_adjust_child_dops_for_redistribution',
                max_tb=debug_max_tb_per_query
            )
        ...
        if debug_dop and _query_id_matches_debug_filter(query_id, debug_query_ids):
            _print_query_dop_debug_snapshot(
                query_id,
                thread_blocks_for_query,
                stage_label='after_adjust_child_dops_for_redistribution',
                max_tb=debug_max_tb_per_query
            )
...
def _query_id_matches_debug_filter(query_id, debug_query_ids):
    if not debug_query_ids:
        return True
    q_str = str(query_id)
    for x in debug_query_ids:
        if str(x) == q_str:
            return True
    return False


def _print_query_dop_debug_snapshot(query_id, thread_blocks_for_query, stage_label, max_tb=20):
    print(f"\n[DEBUG_DOP] Query {query_id} @ {stage_label}")
    shown = 0
    for tid in sorted(thread_blocks_for_query.keys()):
        if shown >= max_tb:
            print(f"  ... truncated, shown {max_tb} thread blocks")
            break
        tb = thread_blocks_for_query[tid]
        candidates = sorted(getattr(tb, 'candidate_optimal_dops', []) or [])
        pred_map = getattr(tb, 'pred_dop_exec_time', {}) or {}
        root_flag = any(getattr(node, 'parent_node', None) is None for node in getattr(tb, 'nodes', []))
        print(
            f"  TB {tid}: root={root_flag}, optimal={getattr(tb, 'optimal_dop', None)}, "
            f"candidate_count={len(candidates)}, candidates={candidates}"
        )
        if candidates:
            sample = []
            for d in candidates[:min(8, len(candidates))]:
                t = pred_map.get(d)
                sample.append(f"{d}:{'None' if t is None else round(float(t), 4)}")
            print(f"    candidate_pred_times(sample)={sample}")
        shown += 1
